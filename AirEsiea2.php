...
if(isset($_POST['Recherche'])) { 
					...
					$jour_a=date("l", $date);
					$jour_num=date("w" ,$date);
					
					$joursem = array('dimanche','lundi', 'mardi', 'mercredi', 'jeudi', 'vendredi', 'samedi');	
					$mois_nom=array('','janvier','février','mars','avril','mai','juin','juillet','aout','septembre','octobre','novembre','decembre');
//echo"Votre jour de départ est le ".$joursem[$jour_num]." ".$_POST['jour']." du mois de ".$mois_nom[$_POST['mois']]." ".$_POST['annee']."";

    $bdd = mysql_connect ('localhost', 'root', '');// On se connecte a la BDD  
    $db_selected = mysql_select_db ('projetbdd', $bdd); // selection de la base projet_bdd
	
	if(!$db_selected) {
		die('Impossible de selectionner la base de données : ' .mysql_error());
	}
// if(isset($_POST['escale'])) {
// $sql = "SELECT DISTINCT v.compagnie as COMPAGNIE, v.num_vol as VOL,a.nom as DEPART,b.nom as ARRIVEE,v.heure_depart, v.temps_vol as DUREE, a.fuseau as fuseau_depart, b.fuseau as fuseau_arrivee FROM vol v 
// INNER JOIN aeroport a ON a.code_aeroport=v.aeroport_depart 
// INNER JOIN aeroport b ON b.code_aeroport=v.aeroport_arrivee 
// WHERE (a.ville = '".$_POST["depart"]."'  or v.aeroport_depart = '".$_POST["depart"]."') and (b.ville = '".$_POST["arrivee"]."' or v.aeroport_arrivee = '".$_POST["arrivee"]."' ) AND INSTR(jours, '".$jour_num."') != 0 AND a.num_vol=";
	// }
$sql = "SELECT DISTINCT v.compagnie as COMPAGNIE, v.num_vol as VOL,a.nom as DEPART,b.nom as ARRIVEE,v.heure_depart, v.temps_vol as DUREE, a.fuseau as fuseau_depart, b.fuseau as fuseau_arrivee FROM vol v 
INNER JOIN aeroport a ON a.code_aeroport=v.aeroport_depart 
INNER JOIN aeroport b ON b.code_aeroport=v.aeroport_arrivee 
...
for($i=0;$i<$reponse = mysql_fetch_assoc($resultat);$i++)
    {
?>
  <td >  <?php  echo $reponse['VOL']; ?> </td>  
...
*/
  ...
  <td >	<?php
	...
	list($heure, $min) = explode(":", $reponse['heure_depart']); //on separe 
						
	$min_final=($reponse['DUREE']+$min) % 60;	

	$heure_final=(int)((($reponse['DUREE']+$min)/60)+$heure+$reponse['fuseau_depart']+$reponse['fuseau_arrivee'])%24;
...
<td> <?php	echo $reponse['COMPAGNIE']; ?> </td>
 </tr> 
